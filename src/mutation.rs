...
fn into_ids_and_counts_by_shopping_cart_item_ids(
    ...
    let message = format!("Response data of `query_counts_by_product_variant_ids` query does not contain a `get_shopping_cart_product_variant_ids_and_counts::GetShoppingCartProductVariantIdsAndCountsEntities::User`, but is another entity: `{:?}`", ids_and_counts_enum);
